...
public class BSPDungeon : MonoBehaviour
{
    ...
    private void ConnectTree(Node node)
    {
        ...
        //2 rooms, now find centre and make centre point;
    ...
    {
        //horizontal segment
        if (from.y == to.y) //if rooms on same horizontal
        {
            int x0 = Mathf.Min(from.x, to.x);
            int w = Mathf.Abs(from.x - to.x);
            var rect = new RectInt(x0 - corridorWidth / 2, from.y - corridorWidth / 2, w, corridorWidth);
            _corridors.Add(rect);
            return;
        }

        //vertical segment
        if (from.x == to.x) //if rooms on same vertical
        {
            int y0 = Mathf.Min(from.y, to.y);
            int h = Mathf.Abs(from.y - to.y);
            var rect = new RectInt(from.x - corridorWidth / 2, y0 - corridorWidth / 2, corridorWidth, h);
            _corridors.Add(rect);
            return;
        }
    }
